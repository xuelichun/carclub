...
public interface NumLevelFeeDao {
	 ...
	 */
	@Insert("insert into num_levelfee values(num_levelfee_seq.nextval,#{nl_id},#{nlf_paytime},#{nlf_mattime},#{nlf_mothtime},#{club_id})")
	public int insertNumLevelFee(NumLevelFee numLevelFee);
	/**
	 * 查询人数级别收费集合（所有的人）
	 * @param page 行数对象
	 * @return 人数级别收费集合
	 ...
	 * @return 功能费用收费集合
	 */
	@Select("select * from num_levelfee")
	 ...
	 * @return 收费到期时间
	 */
	@Select("select nlf_mattime from num_levelfee where club_id=#{club_id} order by nlf_mattime desc")
	public List<String> selectNlfMattime(int club_id);
	/**
	 * 查询人数级别收费表的数据记录数
	 * @return 数据记录数
	 */
	...
	@Select("select * from num_level")
	public List<NumLevel> selectAllNl();
}
