...
public class ClubAdminController {
	...
	public int insertPayCon(PayCon payCon,HttpServletRequest request){
		int row=clubFeeService.insertPayCon(payCon,request);
		return row;
	}
	...
	public int updateActRegOnPay(ActReg actReg,HttpServletRequest request){
		int row=activityService.updateActRegOnPay(actReg,request);
		return row;
	}
	...
	public int updateActRegOnRefund(ActReg actReg,HttpServletRequest request){
		int row=activityService.updateActRegOnRefund(actReg,request);
		return row;
	}
	...
	@RequestMapping("/clubAdminai")
	@ResponseBody
	public int[] yanzhenPre(HttpServletRequest request){
		HttpSession session=request.getSession();
		UserInfo userInfo=(UserInfo) session.getAttribute("user");
		int[] result=new int[2];
		if(userInfo!=null){
			int u_id=userInfo.getU_id();
			int flag=preInfoService.isPresident(u_id);
			 if(flag==2){
				result[0]=2;
				result[1]=u_id;
			}else if(flag==3){
	...
	@RequestMapping("/clubAdminaj")
	@ResponseBody
	public AnnualFee selectAnnualFeeByClubPre(int club_id){
		return feeService.selectAnnualFeeByClubPre(club_id);
	}
	
	@RequestMapping(value="/uploadClubImg",method=RequestMethod.POST)
	@ResponseBody
	public int updateClubByClubPre(MultipartFile club_img,HttpServletRequest request){

		return clubService.updateClubByClubPre(club_img,request);
	}
	
	@RequestMapping("/clubAdminak")
	@ResponseBody
	public List<Function> selectAllFunction(){
		return clubService.selectAllFunction();
	}
	
	@RequestMapping("/clubAdminal")
	@ResponseBody
	public int yearFeeIsOutOfDate(int club_id){
		return clubService.yearFeeIsOutOfDate(club_id);
	}
	
	@RequestMapping("/clubAdminam")
	@ResponseBody
	public List<FunCharge> selectFunChargeOpened(int club_id){
		return clubService.selectFunChargeOpened(club_id);
	}
	
	@RequestMapping("/clubAdminan")
	@ResponseBody
	public List<Function> selectFunctionOpened(int[] fun_id){
		return clubService.selectFunctionOpened(fun_id);
	}
	
	@RequestMapping("/clubAdminao")
	@ResponseBody
	public int jiaoFei(int fun_id,int club_id){
		return clubService.jiaoFei(fun_id, club_id);
	}
	
	@RequestMapping("/clubAdminap")
	@ResponseBody
	public int kaiTongYearFee(int u_id,HttpServletRequest request){
		return clubService.kaiTongYearFee(u_id, request);
	}
}
