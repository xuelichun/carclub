var arr=[];
$(function() {
		//$.getJSON("../clubRank/showRank.do",{},function(data){
	$.ajax({
			type:'post',
			url:'../clubRank/showRank.do',
			async:false,
			success:function(data){
				
				for(var i=0;i<data.length-1;i++){
					arr[i]=data[i].club_pnum;
				}
				arr[data.length-1]=data[data.length-1].club_pnum;
			}
		});
	...
	data = {
			...
			height : 500,
			maxValue : 150,
			xAxis : [ "第一", "第二", "第三", "第四", "第五", "第六","第七"],
			//starRate : [ 0, 2, 5, 16 , 23, 2, 30 ],
			starRate : [],
			//starNum : [ 0, 3, 8, 12, 15, 16, 20 ],
			rectColor : [ "#b5cb85", "#b5cb85", "#b5cb85", "#b5cb85",
					"#b5cb85", "#b5cb85", "#b5cb85" ],
	};
...
//	var str2 = "(%)";
	...
	var drawXAxis = function(xAxis) {
		...
		for (var i = 0; i < len; i++) {
			...
			// 字体居中
			ctx.fillText(xAxis[i], x_scale + (i + 0.5) * stepWidth
					- xAxis[i].length * 14 / 2, canvas.height - y_scale + 24);
	...
	var drawYaxis = function(maxValue, step) {
		...
		for (var i = 1; i <= step; i++) {
			ctx.font = "normal normal bold 14px 微软雅黑";
			// 字体居中
			ctx.fillText(maxValue / step * i, x_scale - 40, canvas.height
					- y_scale - stepHeight * i + 7);
	...
	var drawRect = function(starRate, colorArr) {
		...
		for (var i = 0, len = starRate.length; i < len; i++) {
			...
			ctx.fillRect(stepXArr[i] - stepWidth / 2 - rectWidth / 2,
					canvas.height - y_scale - starRate[i] * heightVal,
					rectWidth, starRate[i] * heightVal);
			ctx.fill();
		}
	}
	...
	var drawBg = function() {
		for (var i = 0; i < stepYArr.length; i++) {
			...
			if (i % 2 == 0) {
				ctx.fillRect(x_scale + 1, stepYArr[i], canvas.width - 2
						* x_scale - stepWidth / 4, stepHeight);
				ctx.fill();
			}
		}
	}
	...
	var mouseMove = function(e) {
		...
		for (var i = 0; i < data.starRate.length; i++) {
			if (x > stepXArr[i] - stepWidth * 3 / 4
					&& x < stepXArr[i] - stepWidth / 4
					&& y < canvas.height - y_scale
					&& y > canvas.height - y_scale - data.starRate[i]
							* heightVal && data.starRate[i] != 0) {
				// 画矩形线
				ctx.beginPath();
				ctx.strokeStyle = "#00b050";
				ctx.lineWidth = 2;
				ctx.rect(stepXArr[i] - stepWidth * 3 / 4 + 2, canvas.height
						- y_scale - data.starRate[i] * heightVal + 2,
						stepWidth / 2 - 4, data.starRate[i] * heightVal - 3);
				ctx.stroke();
				...
				$("#show_num").css(
						{
							...
							"top" : canvas.height - y_scale - data.starRate[i]
									* heightVal - 28
						});
				break;
			} else {
				...
				ctx.fillRect(stepXArr[i] - stepWidth * 3 / 4, canvas.height
						- y_scale - data.starRate[i] * heightVal,
						stepWidth / 2, data.starRate[i] * heightVal);
				ctx.fill();
