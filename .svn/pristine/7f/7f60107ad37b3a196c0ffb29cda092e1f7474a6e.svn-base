...
public interface ClubDao {
	 ...
	 * @return 影响车友会表的行数
	 */
	@Insert("insert into club values(club_seq.nextval,club_no_seq.nextval,#{club_name},#{club_domnam},#{club_pro},#{club_city},#{p_id},#{club_cb},#{club_cd},#{club_desc},#{club_img},#{club_cretime},#{club_maxnum},#{club_tcon},#{club_pnum})")
	...
	@Select("select sum(u_grade) from userinfo where club_id=#{club_id}")
	...
	public Club selectClubByClubIdForClubHome(int club_id);
	
	public int updateClubByClubPre(Club club);
	 ...
	 * @return
	 */
	@Select("select * from(select s.*,rownum as rn from(select * from club where club_pro=#{club_pro} order by club_tcon desc) s)"
			+"where rn between (#{pageIndex}-1)*10+1 and #{pageIndex}*10")
	public List<Club> selectAllClubByPro2(@Param("club_pro")String club_pro,@Param("pageIndex")int pageIndex);
	 ...
	 * @return
	 */
	@Select("select * from(select s.*,rownum as rn from(select * from club where club_city=#{club_city} order by club_tcon desc) s)"
			+"where rn between (#{pageIndex}-1)*10+1 and #{pageIndex}*10")
	public List<Club> selectAllClubByCity2(@Param("club_city")String club_city,@Param("pageIndex")int pageIndex);
	
	
	/**
	 * 根据车的品牌查询车友会集合按总贡献值排名
	 * @param club_cb 车的品牌
	 * @param pageIndex 页数索引
	 * @return 车友会集合
	 */
	@Select("select * from(select s.*,rownum as rn from(select * from club where club_cb=#{club_cb} order by club_tcon desc) s)"
			+"where rn between (#{pageIndex}-1)*10+1 and #{pageIndex}*10")
	public List<Club> selectAllClubByCarBrand2(@Param("club_cb")String club_cb,@Param("pageIndex")int pageIndex);
	 ...
	 * @return 车友会集合
	 */
	@Select("select * from(select s.*,rownum as rn from(select * from club where club_cd=#{club_cd} order by club_tcon desc) s)"
			+"where rn between (#{pageIndex}-1)*10+1 and #{pageIndex}*10")
	public List<Club> selectAllClubByCarDep2(@Param("club_cd")String club_cd,@Param("pageIndex")int pageIndex);
	 ...
	 * @return 车友会集合
	 */
	@Select("select * from(select s.*,rownum as rn from(select * from club where club_pro=#{club_pro} and club_cb=#{club_cb} order by club_tcon desc) s)"
			+"where rn between (#{pageIndex}-1)*10+1 and #{pageIndex}*10")
	public List<Club> selectAllClubByProCB2(@Param("club_pro")String club_pro,@Param("club_cb")String club_cb,
			@Param("pageIndex")int pageIndex);
	 ...
	 * @return 车友会集合
	 */
	@Select("select * from(select s.*,rownum as rn from(select * from club where club_pro=#{club_pro} and club_cd=#{club_cd} order by club_tcon desc) s)"
			+"where rn between (#{pageIndex}-1)*10+1 and #{pageIndex}*10")
	public List<Club> selectAllClubByProCD2(@Param("club_pro")String club_pro,@Param("club_cd")String club_cd,
			@Param("pageIndex")int pageIndex);
	/**
	 * 根据城市和车的品牌查询车友会集合
	 * @param city 城市
	 ...
	 * @return 车友会集合
	 */
	@Select("select * from(select s.*,rownum as rn from(select * from club where club_city=#{club_city} and club_cb=#{club_cb} order by club_tcon desc) s)"
			+"where rn between (#{pageIndex}-1)*10+1 and #{pageIndex}*10")
	public List<Club> selectAllClubByCityCB2(@Param("club_city")String club_city,@Param("club_cb")String club_cb,
			@Param("pageIndex")int pageIndex);
	 ...
	 * @return 车友会集合
	 */
	@Select("select * from(select s.*,rownum as rn from(select * from club where club_city=#{club_city} and club_cd=#{club_cd} order by club_tcon desc) s)"
			+"where rn between (#{pageIndex}-1)*10+1 and #{pageIndex}*10")
	public List<Club> selectAllClubByCityCD2(@Param("club_city")String club_city,@Param("club_cd")String club_cd,
			@Param("pageIndex")int pageIndex);
	
	@Update("update userinfo set club_id=0 where u_id=#{u_id}")
	public int quitCarClub(int u_id);
	
	@Select("select * from function where fun_id=#{fun_id}")
	public Function selectFunction(int fun_id);
	
	@Select("select * from function")
	public List<Function> selectAllFunction();
	
	@Select("select * from fun_charge where club_id=#{club_id}")
