					...
					if(data!=null){
						for(var p in data){
							...
							for(var i=1;i<data[p].length;i++){
								...
								if($("#u_id").val()==u_id){
									context+="<a id='a"+data[p][i].com_id+"' href='javascript:deleteCom(\""+data[p][i].com_id+"\")' style='display:none; padding-left:30px;'>删除</a>" ;
								}
								context+="<br/></div>";
								var com_id=data[p][i].com_id;
								$.ajax({
									type:'post',
									url:'../userDyn/showReply.do',
									data:'com_id='+com_id,
									async:false,
									success:function(data){
										for(var j=0;j<data.length;j++){
											context+="<a href='javascript:jump("+data[j].rp_pid+")'>"+data[j].rp_nick+"</a> 回复  <a href='javascript:jump("+data[j].rp_rpid+")'>"+data[j].rp_rnick+"</a>:"+data[j].rp_text+"<br/>";
										}
										var h=data.length-1;
										context+="<a href='javascript:showReply("+data[h].com_id+")'>回复</a><br/>"+
										"<div style='display:none' id="+data[h].com_id+"> " +
										"<textarea rows='' cols='' id='re"+data[h].com_id+"'>" +
										"</textarea><br/><a  href='javascript:Reply(\""+data[h].rp_pid+"\",\""+data[h].rp_nick+"\",\""+data[h].com_id+"\")'>提交回复</a></div>"
										context+="</div>";	
									}
								});								
							}
