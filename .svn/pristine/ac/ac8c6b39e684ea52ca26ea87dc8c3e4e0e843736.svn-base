	       ...
	       " </div>"+
	        "<div class='content'>"+
	            ...
	            "<table width='100%' border='0' cellspacing='0' cellpadding='0'>"+
	              "<tr>"+
	                "<td id='isOut"+i+"'><a href='carClubActivityDetail.html?club_id="+club_id+"&act_id="+data[i].act_id+"'><img src='images/img_22.jpg' width='206' height='61' /></a></td>"+
	              "</tr>"+
	              "<tr>"+
	                "<td height='20'></td>"+
	              "</tr>"+
	              "<tr>"+
	                "<td align='center' class='baomi' id='baomingrenshu"+i+"'>报名人数：40人</td>"+
	              "</tr>"+
	              "<tr>"+
	                "<td height='20'></td>"+
	              "</tr>"+
...
function selectActivityPage(club_id,page){
	...
	$.getJSON("../clubHomei.do",{"club_id":club_id},function(data){
			...
			for(var i=0;i<data;i++){
				content+="<a href=javascript:showActivity("+1+","+(i+1)+")>"+(i+1)+"</a>";
			}
...
function modifyAct(){
	$.ajax({
		...
		dataType:'json',
		async:false,
		data:"act_id="+act_idArray,
		success:function(data){
			for(var i=0;i<data.length;i++){
				if(data[i]==1){
					$("#isOut"+i).html("<img src='images/act_jiesu.jpg' width='206' height='61' />");
				}
			}
		}
	});
	$.ajax({
		...
		success:function(data){
			for(var i=0;i<data.length;i++){
					$("#baomingrenshu"+i).html("报名人数："+data[i]+"人");
			}
		}
	});
