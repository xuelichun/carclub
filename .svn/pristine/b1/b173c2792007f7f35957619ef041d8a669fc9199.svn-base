...
$(function(){
	...
	$.getJSON("../user/islogin.do",{},function(data){
		if(data==null){
			...
			showFriendsDyn(data.u_id,1);
			nows=1;
			$("#sss").html("第"+nows+"页");
			countSparePyn(data.u_id);
		}
...
function showFriendsDyn(u_id,pageIndex){
	var context="";
//		$.getJSON("../userDyn/showFriendsDyn.do",{"u_id":u_id,"indexPage":indexPage},function(data){
			$.ajax({
				type:'post',
				url:'../userDyn/showFriendsDyn.do',
				data:'u_id='+u_id+'&pageIndex='+pageIndex,
				async:false,
	...
	$.ajax({
		...
		success:function(data){
			...
			if(counts==0){
				...
				showFriendsDyn(u_id,now);
			}else if(counts<nows){
				showFriendsDyn(u_id,counts);
				nows=counts;
				$("#sss").html("第"+nows+"页");
			}else{
				//alert(counts+"---"+nows);
