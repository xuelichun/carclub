...
public class ChaEventController {
	...
	public ChaEvent selectChaEventById(int ce_id){
		System.out.println(ce_id);
		
		System.out.println(chaEventService.selectChaEventById(ce_id));
	...
	@ResponseBody
	public  Object countCha(){
		int a=chaEventService.countCha();
		 return "{\"rs\":\""+a+"\"}";
	}
	
	@RequestMapping("/addTour2")
	@ResponseBody
	public Object addTour2(int u_id,int ce_id){
		if(chaEventService.countChas(u_id, ce_id)>0){
			return "{\"rs\":\"no\"}";
		}else{
			if(chaEventService.addTour2(u_id,ce_id)>0){
				return "{\"rs\":\"ok\"}";
			}else{
				return "{\"rs\":\"failed\"}";
			}
