...
public interface UserDao {
	...
	public List<Visitor> showVisitor(@Param("v_uid2")int v_uid2,@Param("pageIndex")int pageIndex);
	 ...
	 * @return
	 */
	@Select("select an_status from appli_notice where p_id=#{u_id}")
	public String selectAn_status(int u_id);
	/**
	 * 
	 * @param u_id
	 * @return
	 */
	@Select("select count(club_id) from userinfo where u_id=#{u_id}")
	public int selectClub_id(int u_id);
	
	@Select("select club_id from userinfo where u_id=#{u_id}")
	public int selectClub_idForAct(int u_id);
