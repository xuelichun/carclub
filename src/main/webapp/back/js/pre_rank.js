$(function(){
	var pageIndex=1;
	selectPIRank(pageIndex);
});
/**
 * 首页按钮点击事件
 */
function selectFirstPIRank(){
	selectPIRank(1);
}
/**
 * 上一页按钮点击事件
 */
function selectLastPIRank(){
	...
	if(currentPage-1<=0){
		...
		selectPIRank(currentPage-1);
	}
}
/**
 * 下一页按钮点击事件
 */
function  selectNextPIRank(){
	var currentPage=parseInt($("#currentPage").html());
...
function selectEndPIRank(){
	...
	var endPage=parseInt($("#endPage").html());
	selectPIRank(endPage);
}
/**
 * 跳转至指定页面点击事件
 */
function selectIndexPIRank(){
 ...
 */
function selectPIRank(pageIndex){
	var content1=" ";
	...
	"<th>会主贡献</th>"+
	"<th>排行</th></tr>";
	$.getJSON("../backPreInfo/selectAllPreRank.do",{"pageIndex":pageIndex},function(data){
